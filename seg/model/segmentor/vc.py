...
class VCSegmentor(TSSegmentor):
    ...
    def get_losses(
        ...
        if labelled:
            ...
            mean_confidence = torch.mean(torch.max(labels, dim=1)[0])

            global dyn_threshold
            dyn_threshold = (dyn_threshold*self.cfg.solver.optimizer.momentum + (1 - self.cfg.solver.optimizer.momentum)*mean_confidence)

            logger.info(f"Testing Threshold...{dyn_threshold}")
            mask = torch.ge(pseudo_probs_u, dyn_threshold)
            valid_mask = torch.ne(gt_labels, 255) & mask

            _, p_labels1 = torch.max(labels, dim=1)
            _, p_labels2 = torch.max(potential_labels, dim=1)

            low_confidence_mask = torch.lt(pseudo_probs_u, dyn_threshold) & mask
