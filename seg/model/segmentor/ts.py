...
class TSSegmentor(GenericModule):
    ...
    def forward_train(
        ...
        # 2. Unlabeled data training with UniMatch
        # Generate weak and strong views of the unlabeled data
        x_w = torch.stack([data.img for data in data_list_u_weak], dim=0)
        x_s1 = torch.stack([data.img for data in data_list_u_strong], dim=0)
        x_s2 = torch.stack([data.img for data in data_list_u_strong], dim=0)

        # Extract features from the weakly perturbed images
        feat_w = self.segmentor.backbone(x_w)
        feat_fp = self.segmentor.dropout(feat_w)

        # Predictions from different perturbations
        p_w, p_fp = self.segmentor.decoder(feat_w).chunk(2)
        p_s1, p_s2 = self.segmentor.decoder(torch.cat((x_s1, x_s2))).chunk(2)

        # Hard pseudo mask from weak prediction
        mask_w = p_w.argmax(dim=1).detach()

        # Unsupervised loss calculation
        criterion = torch.nn.CrossEntropyLoss()
        p_s = torch.cat((p_s1, p_s2))
        loss_s = criterion(p_s, mask_w.repeat(2, 1, 1))
        loss_fp = criterion(p_fp, mask_w)
        loss_u = (loss_s + loss_fp) / 2.0

        loss_dict = Dict()
        loss_dict.loss_l = loss_dict_l
        loss_dict.loss_u = loss_u+loss_dict_u
